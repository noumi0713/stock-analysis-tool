...
def publish(source: Path, target: Path) -> None:
    ...
    text = ("# 日本株120営業日・チャッピー分析用\n\n"
            ...
            f"3. [最新の株価更新結果]({PUBLIC}/update_status.json)と[データ定義・品質]({PUBLIC}/manifest.json)、[全対象の取得結果]({PUBLIC}/stock_status.csv)、[市場系列の欠損]({PUBLIC}/market_status.json)を確認する。\n"
            f"4. ランキング母集団に該当する銘柄だけ、[全銘柄の120日CSV]({PUBLIC}/equities_120d.csv)または[分析一式ZIP]({PUBLIC}/chatgpt_120d.zip)から抽出し数値処理する。\n"
            f"5. [テーマ所属と関連度]({PUBLIC}/theme_members.csv)、[テーマ推移]({PUBLIC}/themes_120d.csv)、[市場データ]({PUBLIC}/markets_120d.csv)を参照する。\n"
            f"6. [信用需給・空売り・決算・業績修正・時価総額/浮動株・コンセンサスEPS]({PUBLIC}/supplemental.json)と[追加データの取得状況・定義]({PUBLIC}/supplemental_status.json)を読む。各銘柄は supplemental/銘柄コード.json。\n"
            f"7. [計算プログラム]({PUBLIC}/indicators.py)でRSI14・ATR14・MA5/10/20/25/60/75・TOPIX相対強度5/20/60日・売買代金比を120日の生データから毎回計算する。指標値は保存されない。\n"
