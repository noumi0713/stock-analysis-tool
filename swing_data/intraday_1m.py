...
def normalize_symbol(raw: str) -> str:
    ...
    Examples:
      ...
      130A   -> 130A.T
      7203.T -> 7203.T
      AAPL   -> AAPL (explicit non-Japan ticker remains unchanged only if a suffix is present)

    This tool is intended primarily for Tokyo-listed stocks. Bare Japanese-style
    codes are automatically suffixed with .T.
    """
    ...
    # Keep other bare symbols unchanged so the script can still be used for
    # markets such as the US without silently rewriting them.
    return value
...
def fetch_symbol(symbol: str, days: int, session: str, retries: int = 3) -> pd.DataFrame:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if df.empty:
                raise RuntimeError("no rows remained after session filtering")
            return df
...
def morning_low_summary(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    for trading_date, group in morning.groupby("TradingDate", sort=True):
        low_price = pd.to_numeric(group["Low"], errors="coerce").min()
        hits = group[pd.to_numeric(group["Low"], errors="coerce") == low_price]
        if hits.empty:
            continue
        low_ts = hits.index[0]
...
def write_outputs(
    ...
    for requested, symbol in parsed_tickers:
        ...
        try:
            ...
            csv_path = output_dir / f"{safe_name}_1m_{days}d.csv"
    ...
    if not combined.empty:
        combined.to_csv(output_dir / "selected_tickers_1m_combined.csv", index=False, encoding="utf-8-sig")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="1-7",
        help="Number of recent calendar-day-equivalent yfinance periods to request (default: 7)",
    )
    parser.add_argument(
        "--session",
    ...
    for result in results:
        if result.error:
            ...
            print(
                ...
                f"{result.rows} rows ({result.first_timestamp} .. {result.last_timestamp})"
            )
