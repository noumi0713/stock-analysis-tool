...
`bbs-ranking.yml` は毎日07:20 JSTにYahoo!ファイナンスの掲示板投稿数ランキングを全ページ取得します。1日1回の固定スナップショットを `bbs_ranking/daily/YYYY-MM-DD.csv` に保存し、既存日付を上書きしません。履歴から前日順位、順位変化、新規ランクイン、圏外退出、3日・5日推移、連続日数、過去最高順位を再計算します。`rank_change` は前日順位−当日順位で、正数ほど順位上昇です。当日取得に失敗した場合は `bbs_ranking_status.json` を failed にし、当日用の latest/trends を削除して前日データの代用を防ぎます。
...
`bbs_ranking_status.json` が成功し、日付が一致するときだけランキング上位100銘柄を処理する。
