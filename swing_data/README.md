...
`bbs-ranking.yml` は毎日07:20 JSTにYahoo!ファイナンスWeb版の掲示板投稿数（人気）ランキングを全ページ取得します。1日1回の固定スナップショットを `bbs_ranking/daily/YYYY-MM-DD.csv` に保存し、既存日付を上書きしません。人気順位の履歴から、前日順位−当日順位（新規掲載は前日圏外とみなす）で「派生急上昇順位」も再現可能な固定ルールで作成します。Yahoo公式の急上昇ランキングはアプリ限定のため、派生順位を公式アプリ順位とは表示しません。両系列について前日順位、順位変化、新規ランクイン、圏外退出、3日・5日推移、連続日数、過去最高順位を保存します。当日取得に失敗した場合は `bbs_ranking_status.json` を failed にし、当日用の latest/trends を削除して前日データの代用を防ぎます。

主な出力は `bbs_ranking_popular_*.csv`、`bbs_ranking_rising_*.csv`、両方を横持ちした `bbs_ranking_universe_latest.csv` / `bbs_ranking_universe_history.csv` です。既存の `bbs_ranking_latest.csv` / `history.csv` / `trends.csv` は互換性のため人気ランキングを表します。
...
スイング候補の分析母集団は、`bbs_ranking_status.json` が当日 success のときの `bbs_ranking_universe_latest.csv` 掲載銘柄だけです。人気順位と派生急上昇順位を別々に評価します。順位は注目度であり買いシグナルではありません。順位上昇と120日価格・出来高・売買代金・テクニカル・相対強度・IR・ニュースを合わせ、過熱銘柄を除外します。
...
`python -m swing_data.market_consensus --target public-data` は、当日の
`bbs_ranking_status.json` が成功し、日付が一致するときだけ人気・派生急上昇ランキングの和集合（各上位100、重複排除）を処理する。

出力:

- `market_consensus_latest.csv`: 当日の順位順スナップショット
- `market_consensus_history.csv`: 日付・銘柄コード単位の日次履歴
- `market_consensus_status.json`: 成否、分類件数、欠損・取得失敗銘柄
- `market_consensus/daily/YYYY-MM-DD.csv`: 日別スナップショット

今期・来期EPSは現在予想、30日前予想、30日変化率を保存する。
分類は、平均目標株価の上昇余地、評価、今期EPSの30日変化の3要素を固定ルールで集計する。
アナリスト3人未満、必須項目欠損、株式分割、価格・予想期間・目標価格の不整合は
「判定不能」とし、「基本」へ代入しない。

弱気・基本・強気の3価格は、有効な場合はアナリスト目標の低値・平均・高値。
不足時は直近終値±2ATR14（ATR不足時は±10%）を補助シナリオとして出力する。
補助シナリオは市場目標株価ではなく、`scenario_price_method` にその旨を明記する。
