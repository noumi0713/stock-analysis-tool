...
def analyze(data, ts, t_dict):
    ...
    for t in ts:
        try:
            ...
            # --------------------------------------------------
            # 1. 下落率が -7% 〜 -13%（幅を少し広げる）
            # 2. 出来高が -40% 以下（極限から少し緩和）
            # 3. RSIが 35 〜 55（ニュートラル全域）
            # 4. MA25付近（MA25の -3% 〜 +5% の範囲）
            is_dai_honmei = (
            ...
            # 🎯 本命の判定条件（少し厳格化）
            # --------------------------------------------------
            # 1. 下落率が -6% 〜 -15%（浅すぎず深すぎない範囲に絞る）
            # 2. 出来高が -30% 以下（-20%から条件を厳密に）
            # 3. RSIが 30 〜 60（大本命よりは少しだけ広い）
            # 4. 株価がMA25の -3% 以上（サポートをより厳しく判定）
            is_honmei = (
...
if ts_list:
    ...
    st.markdown("条件: 高値から-7%〜-13% / 出来高激減(-40%以下) / RSI35〜55 / 25日線付近")
    if not df_dai.empty:
    ...
    st.markdown("条件: 高値から-6%〜-15% / 出来高低下(-30%以下) / RSI30〜60")
    if not df_hon.empty:
