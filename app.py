...
# --- ページ設定 ---
st.set_page_config(page_title="スイング特化・押し目スクリーナー", layout="wide")
st.title("📉 売り枯れ押し目キャッチ（スイング専用）")
...
def analyze(data, ts, t_dict):
    ...
    for t in ts:
        try:
            ...
            # 下落率の計算（直近高値から現在値が何％落ちているか）
            recent_high = df['RecentHigh'].iloc[-2] # 前日までの20日高値
            ...
            # --------------------------------------------------
            # 1. 下落率が -8% 〜 -12%（標準的な押し目）
            # 2. 出来高が -50% 以下（極端な売り枯れ）
            # 3. RSIが 40 〜 50（過熱感が完全に冷めている）
            # 4. MA25付近（MA25の -2% 〜 +5% の範囲）
            is_dai_honmei = (
                -12.0 <= drop_pct <= -8.0 and
                vol_change_pct <= -50.0 and
                40.0 <= c['RSI'] <= 50.0 and
                c['MA25'] * 0.98 <= c['Close'] <= c['MA25'] * 1.05
            )
            
            # --------------------------------------------------
            # 🎯 本命の判定条件（やや許容範囲の広いスイング押し目）
            # --------------------------------------------------
            # 1. 下落率が -5% 〜 -15%（浅すぎず深すぎない）
            # 2. 出来高が -20% 以下（明確な売り圧力の低下）
            # 3. RSIが 35 〜 55（ニュートラル）
            # 4. 株価がMA25の上にある（強いサポートが機能中）
            is_honmei = (
                -15.0 <= drop_pct <= -5.0 and
                vol_change_pct <= -20.0 and
                35.0 <= c['RSI'] <= 55.0 and
                c['Close'] >= c['MA25'] * 0.95
            )
...
if ts_list:
    st.markdown("直近高値からの「下落率」と「出来高の減少（売り枯れ）」に基づき、スイングトレードに最適な押し目銘柄を厳選します。")
    
    with st.spinner('市場データを取得・解析中...'):
        m_data = fetch_data(ts_list)
    
    df_dai, df_hon = analyze(m_data, ts_list, st.session_state.tickers_dict)
    ...
    st.markdown("条件: 高値から-8%〜-12% / 出来高半減(-50%以下) / RSI40〜50 / 25日線付近")
    if not df_dai.empty:
        st.dataframe(df_dai.style.map(style_eval, subset=['評価']), use_container_width=True, hide_index=True)
    else: 
        st.info("現在、大本命の条件に合致する銘柄はありません。（無理なエントリーを避ける安全な状態です）")

    st.header("🎯 本命（監視すべき優良な押し目）")
    st.markdown("条件: 高値から-5%〜-15% / 出来高低下(-20%以下) / RSI35〜55")
    if not df_hon.empty:
        # 下落率でソート（より-10%に近いものを上に）
