...
def get_base_tickers():
    ...
    7011/三菱重工業 7012/川崎重工業 7014/名村造船所 7003/三井Ｅ＆Ｓ 7018/内海造船 6302/住友重機械工業 9101/日本郵船 9104/商船三井 9107/川崎汽船 7022/サノヤスホールディングス 464A/QPSHD 290A/シンスペ 
...
del_options = {t: f"{t.replace('.T','')}/{st.session_state.tickers_dict[t]}" for t in current_codes}
...
# リセット
if st.sidebar.button("初期リスト(220銘柄)にリセット"):
    st.session_state.tickers_dict = get_base_tickers()
    st.rerun()
    ...
    for ticker in tickers:
        try:
            ...
            res = {
                ...
                "MA25トレンド": ma25_trend, "Vol変化": f"{(vol_ratio-1)*100:+.1f}%"
            }
...
if tickers_list:
    ...
    # 理想的なテクニカル状況の解説 [cite: 1]
    with st.container():
        st.markdown("""
        <div style="background-color: #1e1e1e; padding: 15px; border-radius: 10px; border-left: 5px solid #ff4b4b; margin-bottom: 20px;">
            <h4 style="margin-top:0;">💡 このリストで狙うべき「理想の形」</h4>
            <ul style="font-size: 0.95rem;">
                <li><b>MA25（赤 🔴）</b>が横ばい、もしくは上を向いている土台があること</li>
                <li><b>RSIダイバージェンス</b>：株価が直近安値を更新した際、RSIが前回より切り上がっていること</li>
                <li><b>売り枯れからの急増</b>：出来高が細った後に、本日のVol変化がプラス（急増）していること</li>
                <li><b>トリガー</b>：5日線を実体が力強く上抜けた瞬間がエントリーの急所です</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)

    if not df_now.empty:
        st.dataframe(df_now.style.applymap(style_trend, subset=['MA25トレンド']), use_container_width=True, hide_index=True)
    else:
    ...
    if not df_ready.empty:
        ...
        st.dataframe(df_ready.style.applymap(style_trend, subset=['MA25トレンド']), use_container_width=True, hide_index=True)
