...
# Exclude the first finalized 1000 so this is a genuinely new set.
finalized = set()
prev = ROOT / "discretionary_review_1000_final.csv"
if prev.exists():
...
rows = rows[:LIMIT]
out = ROOT / "discretionary_review_1000_set2_context.csv"
with out.open("w", newline="", encoding="utf-8-sig") as f:
    fields = list(rows[0].keys()) if rows else ["batch","stock_code","long_name","theme_name","cluster"]
...
(ROOT / "discretionary_review_1000_set2_context_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
