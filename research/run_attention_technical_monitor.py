...
def compact_metrics(row: dict, float_shares: float | None = None) -> dict:
    ...
    out["float_turnover_pct"] = round(turnover, 6) if turnover is not None else None; out["float_turnover_evaluable"] = turnover is not None
    return out
...
def monitor_candidate(candidate: dict) -> dict:
    ...
    try: float_shares = float(candidate.get("float_shares")) if candidate.get("float_shares") not in (None, "") else None
    except Exception: float_shares = None
    base = {
        ...
        "minkabu_attention_type": candidate.get("minkabu_attention_type", ""), "minkabu_theme_rank": candidate.get("minkabu_theme_rank"),
        "minkabu_relevance": candidate.get("minkabu_relevance"),
    }
    if idx is None: return {**base, "ok": False, "technical_status": "NO_DISCOVERY_BAR", "error": "no completed market bar existed at attention snapshot"}
    if idx < 24: return {**base, "ok": False, "technical_status": "INSUFFICIENT_HISTORY", "discovery_market_date": bars[idx]["date"], "available_prior_bars": idx + 1, "error": "fewer than 25 bars available at discovery"}
    dbar = bars[idx]
...
def main() -> None:
    ...
    for c in watch.get("primary_watch", []):
        try: items.append(monitor_candidate(c))
        except Exception as e: items.append({"ifis_rank": c.get("ifis_rank"), "stock_code": c.get("stock_code"), "ticker": c.get("ticker"), "company_name": c.get("company_name"), "ok": False, "technical_status": "FETCH_ERROR", "error": str(e)})
    payload = {"status": "complete", "stage": "ENTRY_TIMING_TECHNICAL", "source": "Yahoo Finance chart endpoint", "price_basis": "adjusted close factor applied to OHLC", "point_in_time_rule": "Discovery metrics use only completed daily bars available at the attention snapshot; snapshots before 16:00 JST use the prior market day.", "metrics": ["volume_ratio20","float_turnover_pct(optional)","RSI14","MA5/25/75","MA25 deviation","ATR14%","1/5/10/20-day returns","upper wick ratio","distance from 20-day high","overheat score"], "actionability": "RESEARCH_ONLY", "candidate_count": len(items), "ok_count": sum(1 for x in items if x.get("ok")), "error_count": sum(1 for x in items if not x.get("ok")), "items": sorted(items, key=lambda x: int(x.get("ifis_rank") or 999999))}
