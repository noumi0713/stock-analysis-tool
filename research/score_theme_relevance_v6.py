...
def main() -> None:
    ...
    args = ap.parse_args()
    root = Path(args.dir)
    rev_path = root / f"batch_{args.batch:03d}_revenue_mix_v6.csv"
    prior_path = root / f"batch_{args.batch:03d}_scores_v3.csv"
    if not prior_path.exists():
        prior_path = root / f"batch_{args.batch:03d}_scores_v2.csv"
    out_path = root / f"batch_{args.batch:03d}_scores_v6.csv"
    summary_path = root / f"batch_{args.batch:03d}_score_summary_v6.json"
    ...
    for r in rev_rows:
        ...
        if r.get("share_status") == "non_operating_security":
            review = "non_operating_security"; source = "non_operating_security"
        elif r["theme_name"] in MACRO_THEMES:
            review = "macro_sensitivity_required"; source = "macro_unknown"
        elif r.get("share_status") == "structured_segment_candidate_v6" and share is not None:
            ...
            confidence = "B"
            review = "structured_segment_v6_needs_final_qa"
            source = "structured_segment_v6"
    ...
    bands: dict[str, int] = {}; reviews: dict[str, int] = {}; scored = 0
    for r in out:
        bands[r["band"]] = bands.get(r["band"], 0) + 1
        reviews[r["review_flag"]] = reviews.get(r["review_flag"], 0) + 1
        scored += bool(r["relevance_score"])
    summary = {
        "batch": args.batch,
        ...
        "formula": "55% validated revenue exposure + 30% business directness + 15% growth relevance",
        "status": "research_candidate_v6",
        "warning": "Only themes mapped to one coherent official revenue table are scored. Missing exposure stays unknown.",
    }
