...
def test_attention_discovery_requires_cross_source_stock_theme_membership():
    ifis = [{"ifis_rank":"1","stock_code":"603A","company_name":"Alpha","snapshot_at":"2026-09-04T01:20:00+09:00"},{"ifis_rank":"2","stock_code":"1234","company_name":"Beta","snapshot_at":"2026-09-04T01:20:00+09:00"}]
    minkabu = [{"stock_code":"603A","theme_name":"再生可能エネルギー","attention_type":"急上昇","theme_rank":"2","minkabu_relevance":"88"},{"stock_code":"1234","theme_name":"AI","attention_type":"POPULAR","theme_rank":"1","minkabu_relevance":"99"}]
    master = [{"stock_code":"603A","company_name":"Alpha","theme_name":"再生可能エネルギー","status":"finalized","final_relevance_score":"97","final_band":"主力テーマ","final_confidence":"A"},{"stock_code":"1234","company_name":"Beta","theme_name":"クラウド","status":"finalized","final_relevance_score":"90","final_band":"主力テーマ","final_confidence":"A"}]
    candidates, matches, summary = build_discovery(ifis, minkabu, master, top_n=30)
    assert [x["stock_code"] for x in candidates] == ["603A"]
    assert candidates[0]["buy_decision"] == "NOT_EVALUATED"
    assert candidates[0]["minkabu_attention_type"] == "RISING"
    assert candidates[0]["theme_relevance_score"] == 97
    assert len(matches) == 1 and summary["discovered_count"] == 1


def test_attention_discovery_does_not_double_count_attention_as_score():
    ifis = [{"ifis_rank":"1","stock_code":"1111","company_name":"A","snapshot_at":"2026-09-04T01:20:00+09:00","attention_score":"999"}]
    minkabu = [{"stock_code":"1111","theme_name":"防災","attention_type":"POPULAR","theme_rank":"1","minkabu_relevance":"100"}]
    ...
    candidates, _, _ = build_discovery(ifis, minkabu, master)
