...
def build_watch(rows: list[dict[str, str]]) -> dict:
    ...
    for r in rows:
        ...
        primary.append({
            ...
            "float_shares": as_float(r.get("float_shares"), None),
            "watch_role": "PRIMARY",
        })
    primary.sort(key=lambda x: (x["ifis_rank"], x["stock_code"]))
    return {
        "status": "complete",
        "stage": "POST_IFIS_THEME_DISCOVERY_TECHNICAL_WATCH",
        "selection_rule": "Only IFIS candidates with finalized theme relevance >= 40 enter the technical watch. This is not a buy signal.",
        "active_inputs": ["IFIS top-N attention snapshot", "finalized Kabutan-derived 124-theme relevance master"],
...
def main() -> None:
    ...
    args = ap.parse_args()
    payload = build_watch(read_csv(Path(args.input)))
    out = Path(args.out)
