...
OVR = ROOT / 'discretionary_review_1000_set3_manual_overrides.csv'
OUT = ROOT / 'discretionary_review_1000_set3_final.csv'
SUMMARY = ROOT / 'discretionary_review_1000_set3_final_summary.json'
QA = ROOT / 'discretionary_review_1000_set3_qa_sample.csv'
...
for r in rows:
    ...
    try: prior = float(r.get('prior_relevance_score') or 0)
    except ValueError: prior = 0.0
    if ov:
        ...
        source = 'chatgpt_confirmed_official_context' if conf in ('A','B') else 'chatgpt_discretion_existing_context'
    out.append({
        'set':'3','batch':r['batch'],'stock_code':r['stock_code'],'long_name':r['long_name'],'theme_name':r['theme_name'],'cluster':r['cluster'],
        'revenue_share_pct':'','final_relevance_score':f'{score:.1f}','final_band':band(score),'final_confidence':conf,
        'decision_source':source,'decision_reason':reason,'source_url':source_url,
        'current_business_score':r.get('current_business_score',''),'growth_relevance_score':r.get('growth_relevance_score',''),
        'prior_relevance_score':r.get('prior_relevance_score',''),'status':'finalized_discretionary_v1'
    })
...
summary = {
    'set':3,'rows':len(out),'manual_web_overrides':manual,
...
print(json.dumps(summary, ensure_ascii=False)); print('qa_rows',len(sample))
