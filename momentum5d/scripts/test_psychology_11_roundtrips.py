...
def enrich(g: pd.DataFrame) -> pd.DataFrame:
    ...
    g["participation"] = turn / g.turnover_med20.replace(0, np.nan)
    ...
    g["blue_entry"] = (
        g.quality & g.liquid & high_part & (long_trend | ipo_trend)
        & g.sentiment.between(60, 80) & g.rsi14.between(52, 72)
        & g.ma_gap.between(-0.01, 0.12) & (breakout | pullback | ipo_breakout)
    )
    g["contra_entry"] = (
        g.quality & g.liquid & high_part & (g.sentiment <= 35) & (g.rsi14 <= 38)
        & ((g.ret5 <= -0.08) | (g.drawdown_high <= -0.15))
        & (g.vol_ratio >= 1.30) & (c > g.open) & (c > c.shift(1)) & (g.day_pos >= 0.65)
    )
    g["euphoria"] = (
        g.quality & g.liquid & (g.sentiment >= 80) & (g.rsi14 >= 72)
        & ((g.ma_gap >= 0.12) | (g.ret10 >= 0.15))
        & ((g.vol_ratio >= 1.30) | (g.ret5 >= 0.10)) & (g.drawdown_high >= -0.04)
    )
    return g
...
def simulate(g: pd.DataFrame, mode: str) -> list[dict[str, Any]]:
    ...
    while i < len(g) - 1:
        ...
        if pos is None:
            ...
            allowed = sig and (mode == "COMBINED" or mode == sig)
            if allowed:
                er = g.iloc[i + 1]
                pos = {
                    "signal": sig,
            ...
            elif pos["signal"] == "BLUE" and pd.notna(r.ma25) and r.close < r.ma25:
            ...
            if exit_reason and i < len(g) - 1:
                ...
                gross = xp / ep - 1
                net = (xp * (1 - COST_ONE_WAY)) / (ep * (1 + COST_ONE_WAY)) - 1
    ...
    if pos is not None:
        xr = g.iloc[-1]
        ep, xp = pos["entry_price"], float(xr.close)
        path = g.iloc[pos["entry_i"]:]
        trades.append({
            **pos,
            "exit_signal_date": xr.date,
            "exit_date": xr.date,
            "exit_price": xp,
            "exit_reason": "DATA_END",
            "hold_days": int(len(g) - 1 - pos["entry_i"]),
...
def summarize(t: pd.DataFrame) -> dict[str, Any]:
    if t.empty:
        return {"n": 0}
    eq = (1 + t.net_return).cumprod()
    peak = eq.cummax()
    dd = eq / peak - 1
    losses = t.loc[t.net_return < 0, "net_return"]
    wins = t.loc[t.net_return > 0, "net_return"]
    pf = float(wins.sum() / abs(losses.sum())) if not losses.empty and abs(losses.sum()) > 0 else None
    return {
        "n": int(len(t)),
        "mean_net": float(t.net_return.mean()),
        "median_net": float(t.net_return.median()),
        "win_rate": float((t.net_return > 0).mean()),
        "pf": pf,
        "compound": float(eq.iloc[-1] - 1),
        "max_dd": float(dd.min()),
...
def main() -> None:
    ...
    out: dict[str, Any] = {
        "meta": {
            "data_start": m["dates"][0],
            "data_end": m["dates"][-1],
            "codes": sorted(raw),
            "entry_execution": "next day open",
            "exit_execution": "next day open",
    ...
    for mode in ["BLUE", "CONTRARIAN", "COMBINED"]:
        mt = t[t["mode"] == mode].copy() if not t.empty else pd.DataFrame()
        out["overall"][mode] = summarize(mt)

    for code in sorted(raw):
        out["by_code"][code] = {}
        for mode in ["BLUE", "CONTRARIAN", "COMBINED"]:
            mt = t[(t["code"] == code) & (t["mode"] == mode)].copy() if not t.empty else pd.DataFrame()
            out["by_code"][code][mode] = summarize(mt)

    a.output_json.parent.mkdir(parents=True, exist_ok=True)
    a.output_json.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")

    lines = [
        "# 11 teaching stocks psychology round-trip test", "",
        f"- Data: {m['dates'][0]} to {m['dates'][-1]}",
        "- Entry at next trading day open after signal",
        "- Exit at next trading day open after exit condition",
        "- Cost: 0.10% each side",
    ...
    for mode in ["BLUE", "CONTRARIAN", "COMBINED"]:
        s = out["overall"][mode]
        lines.append(
            f"| {mode} | {s.get('n', 0)} | {pct(s.get('mean_net'))} | {pct(s.get('median_net'))} | "
    ...
    lines += [
        "", "* Compound/Max DD are sequential trade equity statistics, not a capital-allocation portfolio simulation.",
        "", "## By code (COMBINED)", "",
        "| Code | N | Mean net | Win | PF | Compound | Max DD |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for code in sorted(raw):
        s = out["by_code"][code]["COMBINED"]
        lines.append(
            f"| {code} | {s.get('n', 0)} | {pct(s.get('mean_net'))} | {pct(s.get('win_rate'))} | "
            f"{num(s.get('pf'))} | {pct(s.get('compound'))} | {pct(s.get('max_dd'))} |"
        )

    lines += [
        "", "## Exit rules", "",
        "- BLUE: euphoria -> exit; otherwise close below MA25 -> exit; otherwise max 10 days.",
        "- CONTRARIAN: euphoria -> exit; otherwise sentiment >=60 -> exit; otherwise max 10 days.",
        "- COMBINED: accepts either entry and applies the matching exit rule.",
        "", "## Caveat", "",
        "These 11 stocks were selected after observing their charts, so this is an in-sample teaching-stock audit and must not be treated as OOS evidence.",
    ]
