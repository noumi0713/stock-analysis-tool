...
現在の価格・出来高の取得源は `yfinance` です。個人研究・非公開利用だけを想定し、直近3年（1,096日）をローリング保存します。東証銘柄コード一覧にはJPXから更新した銘柄マスタを使い、日足価格はYahoo Finance由来です。従来のJ-Quants取得コマンドとバックテスト実装は互換性のため残しています。
...
初押しと投げ売り反転の本番候補ルールは
`app/resources/strategy_specs/live_v1_2026-08-31.json` に統一し、SHA-256で変更を検知します。
...
A/B/C/見送り判断は `scripts/record_secondary_decisions.py` で追記専用保存します。
判定情報の締切、判定時刻、翌営業日寄り時刻と、地合い・業種/テーマ・個別材料・
決算/イベント・需給・当時の株価のスナップショットを必須にし、未来情報を拒否します。
一次候補のスナップショットを保持し、実際に買わなかった候補も含めて
`scripts/update_candidate_outcomes.py` が投げ売り反転10営業日、初押し15営業日の
凍結済み出口ルールで追跡します。

`scripts/evaluate_secondary_decisions.py` は戦略を混ぜず、Aのみ、A+B、A+B+C、一次全件を
比較します。件数、勝率と95%信頼区間、平均・中央値・期待値、PF、最大DD、最大連敗、
±5%/−8%到達率、総収益、資金効率、機会減少、除外利益と回避損失を出力します。
完了サンプル50件未満は判断保留とし、期待値を維持または向上しながら最大DDを
低下させた場合だけ二次選別を採用します。

日次シグナル、A/B/C記録、公式保有期間追跡、認証バックテストには、最終取引日、取得時刻、
...
初回取得と毎日の差分更新は同じコマンドです。既存データの最終日から10日重ねて取得し、訂正値を上書きしたあと、基準日から1,096日より古い行を削除します。保存期間を拡張した場合は、既存データの開始日を確認して不足期間を自動的にバックフィルします。
...
出来高0または価格が無効な行を取引不能として除外します。現在の主ランキングは、翌営業日始値から10営業日以内に日中高値+5%へ到達する確率を目的変数にしています。急落継続・投げ売り反転・緩やかな底固めの3形状と売買代金2億円以上を母集団に、形状別のロジスティック回帰とHistGradientBoosting、上昇確率、−5%・−8%下落確率、期待損益、翌日ギャップ、地合い条件を比較します。モデルと閾値はウォークフォワード前半だけで選択し、後半は条件を固定した未使用検証です。主ランキングは期待値順に1日最大1銘柄、該当なしの日は空ランキングとします。従来の5営業日版は比較用データとして残し、決算・重要指標リスク制限も引き続き適用します。
...
1. 単体テストと静的検査
2. 東証銘柄の直近3年（1,096日）の日足を取得
3. 当日の5分足を前場11:30または大引け15:30まで集計し、当日の日足へ反映
4. 品質検査と+5%局面分析
5. 全銘柄の日足をGitHubへ保存せず、厳選候補0〜1銘柄の直近60営業日だけを表示用JSONへ保存
...
本番ランキングは平均3営業日に1回の検知を目標に選んだRSI(14)条件を使用します。RSI 25〜35、前日比-3〜0%、5日騰落-15〜+2%、出来高比1.5倍以上、売買代金3億円以上、ATR 2〜12%、25日線下の陽線を抽出し、出来高比順で1日最大3銘柄を翌営業日始値の候補として公開します。
