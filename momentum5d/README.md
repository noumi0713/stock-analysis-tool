...
分析は予測モデルを学習しません。出来高0または価格が無効な行を取引不能として除外し、過去5営業日以内に高値が+5%へ到達した場面を集計します。現在のランキングは、従来の上昇前の仕込み形状60%と、個人投資家フローの代理スコア40%を組み合わせます。Prime全体の5日上昇銘柄比率が50%以下、または20日リターン中央値が0以下の日は候補を出しません。翌営業日始値が前日終値比+1.5%超または-3%未満なら新規購入を見送ります。スコアは上昇確率ではありません。
...
`yahoo-backtest-retail` は従来の仕込み形状、酒田五法、個人投資家フロー単独、採用した混合方式を、翌営業日始値エントリー、+5%利確、5営業日目終値決済、往復コスト0.2%、5分割資金の同一条件で比較します。結果はJSON・CSV・Markdownを `outputs/retail_flow_backtest/` に保存します。損切りはこの比較には入れていません。
