...
分析は予測モデルを学習しません。出来高0または価格が無効な行を取引不能として除外し、過去5営業日以内に高値が+5%へ到達した場面を集計します。最新ランキングは「上昇前の仕込み候補」に目的を限定し、1日+2.5%・5日+4%・20日+15%を超えた銘柄や20日高値を更新済みの銘柄を除外します。そのうえで、出来高の立ち上がり、適度な10日ボラティリティ、上昇日の出来高比率、20日高値までの距離、初動の5日リターンを組み合わせた説明可能な `setup_score` を作ります。さらに17業種ごとの5日・20日・60日リターン中央値と上昇銘柄比率から業種トレンドを計算し、最終順位は `setup_score` 75%・業種トレンド25%で決めます。Prime全体の5日上昇銘柄比率が50%以下、または20日リターン中央値が0以下の日は候補を出しません。翌営業日始値が前日終値比+1.5%超または-3%未満なら、ランキングに含まれていても新規購入を見送る運用ルールです。これは上昇確率ではありません。
