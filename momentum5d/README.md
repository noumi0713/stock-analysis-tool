...
分析は予測モデルを学習しません。出来高0または価格が無効な行を取引不能として除外し、過去5営業日以内に高値が+5%へ到達した場面を集計します。最新ランキングは「上昇前の仕込み候補」に目的を限定し、1日+2.5%・5日+4%・20日+15%を超えた銘柄や20日高値を更新済みの銘柄を除外します。そのうえで、10日値幅の収縮、5日持ち合い、上昇日の出来高比率、出来高急増前、20日高値までの距離を組み合わせた説明可能な `setup_score` で順位化します。これは上昇確率ではありません。
