...
分析は予測モデルを学習しません。出来高0または価格が無効な行を取引不能として除外し、過去5営業日以内に高値が+5%へ到達した場面を集計します。テクニカル判断は酒田五法へ統一し、買い型の「逆三山・三川明けの明星・三空叩き込み・赤三兵・上げ三法」と、売り型の「三山・三川宵の明星・三空踏み上げ・黒三兵・下げ三法」を当日までのOHLCだけで検出します。買い型があり売り型がない銘柄を対象に、酒田五法スコア75%・17業種トレンド25%で順位を決めます。Prime全体の5日上昇銘柄比率が50%以下、または20日リターン中央値が0以下の日は候補を出しません。翌営業日始値が前日終値比+1.5%超または-3%未満なら新規購入を見送ります。スコアは上昇確率ではありません。
...
`yahoo-backtest-sakata` は従来の仕込み形状スコアと酒田五法を、翌営業日始値エントリー、+5%利確、5営業日目終値決済、往復コスト0.2%、5分割資金の同一条件で比較し、JSON・CSV・Markdownを `outputs/sakata_backtest/` に保存します。
